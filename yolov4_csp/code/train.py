...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, nndct_quant = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.total_batch_size, opt.weights, opt.global_rank, opt.nndct_quant
    ...
    if nndct_quant:
        ...
        # embed()
        # print(model)
        model.forward = partial(model.forward, augment=False, quant=True)
    ...
    # }]
    # optimizer = AdamW(
    #     param_groups, hyp['lr0'], weight_decay=hyp['weight_decay'], betas=(hyp['momentum'], 0.999))
    pg0, pg1, pg2 = [], [], []  # optimizer parameter groups
    pg_qat = []
    for k, v in dict(model.named_parameters()).items():
        if nndct_quant and 'threshold' in k:
            pg_qat.append(v)
        elif '.bias' in k:
            pg2.append(v)  # biases
    ...
    if opt.adam:
        if opt.adamw:
            optimizer = AdamW(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
        else:
    ...
    optimizer.add_param_group({'params': pg_qat, 'lr': hyp['lr0'] * 100, 'name': 'threshold'})  # add pg2 (biases)
    logger.info('Optimizer groups: %g .bias, %g conv.weight, %g other' % (len(pg2), len(pg1), len(pg0)))
    del pg0, pg1, pg2, pg_qat
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(test_path, imgsz_test, batch_size*2, gs, opt,
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=not nndct_quant,
