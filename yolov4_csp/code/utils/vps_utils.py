...
def compute_two_points(degree, point1, point2):
    p1_p2 = point2 - point1 #[w2-w1, h2-h1]
    p1_p2_norm = np.sqrt(p1_p2[0] ** 2 + p1_p2[1] ** 2)     # distance
    # print('The distance between point1 and point2:{}'.format(p1_p2_norm))
    if p1_p2_norm < 200:
        depth = 250
    else:
        depth = 125
    p1_p2_unit = p1_p2 / p1_p2_norm     # [w2-w1, h2-h1] / distance----()
...
def bbox_wh_iou(wh1, wh2):
    ...
    Args:
        wh1(2):anchor w & h
        wh2(batch_targets, 2): 2 for w and h
    Returns:
        iou (1, batch_targets): every target's iou with this anchor
    """
    # (batch_targets, 2)--->(2, batch_targets) [i][j]-->[j][i]
    wh2 = wh2.t()
    w1, h1 = wh1[0], wh1[1]
    w2, h2 = wh2[0], wh2[1]
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres):
    """
    Args:
        pred_boxes(BS, 3, Grid_W, Grid_H, 4):3 for anchors, 4 for cx, cy, w, h
        pred_cls (BS, 3, Grid_W, Grid_H, 4): 4 for cls0, cls1, cls2, cls3
        target(BS, num_targets, 6):6 for class_id, cx, cy, w, h, vacant;coordinates are normalized
    Returns:
        obj_mask(BS, 3, Grid_W, Grid_H): obj_mask[:][i][x][y]=1: means target in gridcell[x][y] best fits anchor[i] 
            or the i-th anchor is responsible for the target in grid_cell[i][j]
        noobj_mask(BS, 3, Grid_W, Grid_H): means this anchor is responsible for nothing.
        iou_scores(BS, 3, Grid_W, Grid_H): The IoU between targets and 
            their corresponding anchor which are offset by the predictions
        class_mask(BS, 3, Grid_W, Grid_H): 0 or 1,means which best anchor's class are predicted right
    """
    # change target from (BS, num_targets, 6) to (batch_targets, 7)
    # 7 : [batch_idx, class_id, cx, cy, w, h, vacant]
    device = pred_boxes.device
    target = list(target)
    num_target = 0
    for batch_idx in range(len(target)):
        num_target += len(target[batch_idx])
    batch_target = torch.FloatTensor(num_target, 1 + 6).fill_(0).to(device)
    num = 0
    for batch_idx in range(len(target)):
        for label_idx in range(len(target[batch_idx])):
            batch_target[num] = torch.cat((torch.Tensor([batch_idx]).to(device), target[batch_idx][label_idx].to(device)))
            num += 1
    ByteTensor = torch.cuda.ByteTensor if pred_boxes.is_cuda else torch.ByteTensor
    FloatTensor = torch.cuda.FloatTensor if pred_boxes.is_cuda else torch.FloatTensor

    nB = pred_boxes.size(0) # batch_size
    assert nB == len(target), "batch_size process wrong:251"
    nA = pred_boxes.size(1) # 3
    nC = pred_cls.size(-1)  # 4:right, acute, xxx, markings
    ...
    # get (batch_targets, 4):[cx, cy, w, h]
    # print(batch_target.shape)
    target_boxes = batch_target[:, 2:6] * nG
    # assert target_boxes.max() < 1, "labels wrong"
    gxy = target_boxes[:, :2]   # (batch_targets, 2):[cx, cy]
    gwh = target_boxes[:, 2:]   # (batch_targets, 2):[w, h]
    # Get anchors with best iou
    # ious (3, batch_targets): ious[i][j] means iou between anchor_i and target_j
    ious = torch.stack([bbox_wh_iou(anchor, gwh) for anchor in anchors])
    # Get the max value along row
    best_ious, best_n = ious.max(0) #(1, batch_targets)
    # Separate target values
    # b(1, batch_targets):batch_idx
    # target_labels(1, batch_targets): class_idx
    b, target_labels = batch_target[:, :2].long().t()
    gx, gy = gxy.t()
    gw, gh = gwh.t()
    gi, gj = gxy.long().t()
    # print(b)
    # Set masks
    assert b.max() <= nB, "batch_size"
    assert best_n.max() <= nA, "anchor"
    assert gj.max() <= nG, "grid_size"
    assert gi.max() <= nG, "grid_size"
    # 
    ...
    # pred_boxes[b, best_n, gj, gi] are the predictions of the anchors which are responsible for targets
