...
class FeatureConcat(nn.Module):
    ...
    def forward(self, x, outputs):
        return self.cat([outputs[i] for i in self.layers], 1) if self.multiple else outputs[self.layers[0]]
...
class FeatureConcat2(nn.Module):
    ...
    def forward(self, x, outputs):
        return self.cat([outputs[self.layers[0]], outputs[self.layers[1]].detach()], 1)
...
class FeatureConcat3(nn.Module):
    ...
    def forward(self, x, outputs):
        return self.cat([outputs[self.layers[0]], outputs[self.layers[1]].detach(), outputs[self.layers[2]].detach()], 1)
...
class FeatureConcat_l(nn.Module):
    ...
    def forward(self, x, outputs):
        return self.cat([outputs[i][:,:outputs[i].shape[1]//2,:,:] for i in self.layers], 1) if self.multiple else outputs[self.layers[0]][:,:outputs[self.layers[0]].shape[1]//2,:,:]
...
class WeightedFeatureFusion(nn.Module):  # weighted sum of 2 or more layers https://arxiv.org/abs/1911.09070
    ...
    def forward(self, x, outputs):
        ...
        # assert not torch.is_tensor(nx), "nx is a tensor"
        # assert isinstance(nx, int), "nx must be a int"
        for i in range(self.n - 1):
            a = outputs[self.layers[i]] * w[i + 1] if self.weight else outputs[self.layers[i]]  # feature to add
            # print(outputs)
            na = a.shape[1]  # feature channels
            # assert not torch.is_tensor(na), "na is a tensor"
            # assert isinstance(na, int), "na must be a int"
            ...
            if nx == na:  # same shape
                ...
                x[:, :na] = self.add(x[:, :na], a)#x[:, :na] + a  # or a = nn.ZeroPad2d((0, 0, 0, 0, 0, dc))(a); x = x + a
            else:  # slice feature
                x = self.add(x, a[:, :nx])#x + a[:, :nx]
