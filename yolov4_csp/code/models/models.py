...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if mdef['activation'] == 'leaky':  # activation study https://github.com/ultralytics/yolov3/issues/441
                modules.add_module('activation', nn.LeakyReLU(negative_slope=26./256))
            ...
            print("Use DeformConv2d!!!!!!")
            assert mdef['type'] != 'deformableconvolutional'
            bn = mdef['batch_normalize']
            filters = mdef['filters']
            k = mdef['size']  # kernel size
            stride = mdef['stride'] if 'stride' in mdef else (mdef['stride_y'], mdef['stride_x'])
            ...
            # modules.add_module('FeatureConcat', FeatureConcat(layers=layers))
            ...
            # modules.add_module('FeatureConcat2', FeatureConcat2(layers=layers))
            ...
            # modules.add_module('FeatureConcat3', FeatureConcat3(layers=layers))
            ...
            # modules.add_module('FeatureConcat_l', FeatureConcat_l(layers=layers))
    ...
    for i in routs:
        routs_binary[i] = True
    return module_list, routs_binary, yololayer_cfg_index


class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        """
        a post training layer which didn't include learned parameters
        1. change the shape of the output from (bs, 255, h, w) to (bs, 3, h, w, 85)
        2. sigmoid
        3. + grid_w&h
        """
    ...
    def post_process(self, p):
        ...
        # return p if self.training else (io.view(bs, -1, self.no), p)
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg, img_size=(416, 416), verbose=False):
        ...
        self.module_list, self.routs, self.yololayer_num = create_modules(self.module_defs, img_size, cfg)
        # print(self.module_list, self.routs)
        ...
        # self.yololayer_num = [144, 159, 174]
        self.stride = torch.tensor([256 / x.shape[-2] for x in self.forward(torch.zeros(1, 3, 256, 256))])  # forward

    def forward(self, x, augment=False, verbose=False, quant=False, debug=False):
        x = self.quant(x)
        if not augment:
            return self.forward_once(x, quant=quant)
    ...
    def forward_once(self, x, augment=False, verbose=False, quant=False, debug=False):
        # if debug:
        #     from IPython import embed
        #     embed()
        img_size = x.shape[-2:]  # height, width
        ...
        for i, module in enumerate(self.module_list):
            ...
            if name in ['WeightedFeatureFusion', 'FeatureConcat', 'FeatureConcat2', 'FeatureConcat3', 'FeatureConcat_l', 'ScaleChannel', 'ScaleSpatial']:  # sum, concat
                ...
                # print(i, module, len(self.module_list), out)
        ...
        x = list(yolo_out)
        if not quant:
            # print("-----------------------------------not quant, training:{}----------------------------------".format(self.training))
            z = []
            z.append(self.module_list[self.yololayer_num[0]].post_process(x[0]))
            z.append(self.module_list[self.yololayer_num[1]].post_process(x[1]))
            z.append(self.module_list[self.yololayer_num[2]].post_process(x[2]))
            if self.training:
                return z
            else:
                te, tr = zip(*z)
                return torch.cat(te, 1), tr

              # # cat yolo outputs
              #   (torch.cat(z, 1), x)
        else:
            # print("-----------------------------------quant, training:{}----------------------------------".format(self.training))
            if self.training:
                z = []
                z.append(self.module_list[self.yololayer_num[0]].post_process(x[0]))
                z.append(self.module_list[self.yololayer_num[1]].post_process(x[1]))
        ...
        # return (z, x) if not quant else x
