...
from ultralytics.yolo.utils.checks import check_imgsz, check_requirements, check_suffix
...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        do_dump = self.args.dump_xmodel or self.args.dump_onnx
        if self.training:
            self.device = trainer.device
            self.data = trainer.data
            model = trainer.ema.ema or trainer.model if not self.args.nndct_quant else trainer.ori_model_ema
            self.args.half = self.device.type != 'cpu' and not self.args.nndct_quant # force FP16 val during training
            ...
            if self.args.nndct_quant:
                ...
                assert self.args.half is False, "Invalid settings for nndct quant"
                if do_dump:
                    assert self.args.quant_mode == 'test', "Quant model should be 'test' for dumping xmodel"
                    assert self.args.batch == 1, "Dump xmodel only support batch size 1"
            self.device = select_device(self.args.device, self.args.batch)
            self.args.half &= self.device.type != 'cpu' and not self.args.nndct_quant
            model = AutoBackend(model, device=self.device, dnn=self.args.dnn, fp16=self.args.half, fuse=not self.args.nndct_quant)
            self.model = model
            ...
            if not pt or self.args.nndct_quant:
        ...
        if self.args.nndct_quant:
            ...
            input_tensor = torch.zeros(1, 3, self.args.imgsz, self.args.imgsz).to(self.device).type_as(next(model.parameters()))
            if self.training:
                model.forward = partial(model.forward, quant=True)
            else:
                model.model.forward = partial(model.model.forward, quant=True)
            weights = trainer.weights if self.training else model.model.pt_path

            if self.training:
                assert self.args.quant_mode == 'test'
                output_dir = weights
            else:
            ...
            if self.training:
                module = model
            # elif self.args.dump_xmodel:
            #     module = model.model.cpu()
            #     input_tensor = input_tensor.cpu()
            else:
                module = model.model
            quantizer = torch_quantizer(quant_mode=self.args.quant_mode,
                                        bitwidth=8, #nndct_bitwidth
                                        module=module, #model if self.training else model.model,
            ...
            if self.training:
                ...
                post_method = model.model.model[-1].post_process
        ...
        for batch_i, batch in enumerate(bar):
            ...
            if do_dump:
                break
        if self.args.nndct_quant and self.args.quant_mode == 'calib':
            quantizer.export_quant_config()
        if self.args.dump_xmodel:
            quantizer.export_onnx_model(output_dir=output_dir.as_posix(), verbose=True, dynamic_batch=True)
            quantizer.export_xmodel(output_dir=output_dir.as_posix(), deploy_check=True)
            return
        if self.args.dump_onnx:
            quantizer.export_onnx_model(output_dir=output_dir.as_posix(), verbose=True, dynamic_batch=True)
            return   
