...
def attempt_load(weights, map_location=None, inplace=True, fuse=True, force_reexport_deployable_model=False, imgsz=640, with_postprocess=False):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        _model.load_state_dict(_model_ckpt.state_dict(), strict=False)
        ...
        if 'qat_model_quant_info' in ckpt:
            if force_reexport_deployable_model:
                ...
                _model.forward = partial(_model.forward, augment=False, profile=False, visualize=False, quant=True, with_postprocess=False, force_inference=True)
