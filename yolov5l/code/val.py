...
def run(data,
    ...
    do_dump = dump_xmodel or dump_onnx or dump_torch_script
    if nndct_quant:
    ...
    if training and not nndct_quant:  # called by train.py
        ...
        if training:
            ...
            model = attempt_load(weights, map_location=device, fuse=not nndct_quant, force_reexport_deployable_model=not training, imgsz=imgsz, with_postprocess=with_postprocess)  # load FP32 model
            # Data
    ...
    if nndct_quant:
        ...
        if with_postprocess:
            # with postprocess
            model.forward = partial(model.forward, quant=True, with_postprocess=True)
            if training:
    ...
    if dump_onnx:
        quantizer.export_onnx_model(output_dir=output_dir.as_posix(), verbose=True, dynamic_batch=True)
        return   
    if dump_torch_script:
        quantizer.export_traced_torch_script(output_dir=output_dir.as_posix())
        return
