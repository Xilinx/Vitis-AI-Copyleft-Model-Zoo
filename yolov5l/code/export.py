...
from utils.datasets import LoadImages, LoadImagesAndLabels, create_dataloader
...
def export_nndct(model, im, file, data, ncalib, nndct_bitwidth=8, prefix=colorstr('NNDCT:')):
    ...
    try:
        ...
        input_tensor = (im,)
        output_dir = (file.parent / 'nndct_quant')
        output_dir.mkdir(parents=True, exist_ok=True)

        model.forward = partial(model.forward, quant=True, with_postprocess=False)
        ...
        assert imgsz[0] == imgsz[1], "Img should be squared"
        gs = max(int(model.stride.max()), 32)  # grid size (max stride)
        data = check_dataset(data)
        dataloader = create_dataloader(data['val'], imgsz[0], batch_size, gs, single_cls=False, pad=0.5, rect=False,
                                        prefix=colorstr(f'calib: '), workers=8)[0]

        print(f'\n{prefix} starting do calibration...')
        if ncalib == -1:
            ncalib = len(dataloader)
        for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, total=ncalib)):
            img = img.to(im.device, non_blocking=True).float()
        ...
        quantizer = torch_quantizer(quant_mode='test',
                                    ...
                                    output_dir=output_dir.as_posix())
        quant_model = quantizer.quant_model
        dataloader = create_dataloader(data['val'], imgsz[0], 1, gs, single_cls=False, pad=0.5, rect=False,
                                        prefix=colorstr(f'calib: '), workers=8)[0]
        ...
        f = output_dir / 'Model_0_int.xmodel'
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    nndct_exports = 'nndct' in include
    ...
    model = attempt_load(weights, map_location=device, inplace=True, fuse=not nndct_exports)  # load FP32 model
    # from models.yolo import Model
    # nc = check_dataset(data)['nc']
    # ckpt = torch.load(weights, map_location=device) 
    # model = Model(ckpt['model'].yaml, ch=3, nc=nc).to(device)
    # model.load_state_dict(model_import.state_dict())
    ...
    if 'nndct' in include:
        export_nndct(model, im, file, data, ncalib=-1)
...
def parse_opt():
    ...
    parser.add_argument('--include', nargs='+',
                        ...
                        help='available formats are (torchscript, onnx, coreml, saved_model, pb, tflite, tfjs, nndct)')
